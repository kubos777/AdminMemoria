...
memoria *ejecutar_round_robin(memoria *memoria, lista *procesos)
{
	...
	if (ejecutar->desplazamiento % 15 == 0) 	
	{//Si ya se todas las instrucciones en un bloque
		if (ejecutar->paginas >= 15) //Si aún hay páginas por cargar
		{
...
memoria *cambiar_memoria_a_FIFO(memoria *rr)
{
	...
	enqueue(cargar->pid, cargar->size, cargar->paginas, cargar->inst_ejec, cargar->bloque, cargar->desplazamiento, fifo);
	...
	{
		printf("Se reccorió %d páginas al proceso con PID %d para encolar al proceso con PID %d\n", recorre->paginas, recorre->next->pid, recorre->pid);
		recorre = recorre->next;
	}
	return fifo;
}
...
void ejecutar_fifo(memoria *fifo)
{
	...
	{
		printf("Terminó la ejecución del proceso con PID %d. Se quedó en:\n", ejecutar->pid);
		printf("Dirección virtual: (%d, %d). ", ejecutar->paginas-1, ejecutar->size%15);
		printf("Dirección real: %d\n", 1000 + (ejecutar->paginas-1)*15 + ejecutar->size%15);
		printf("Se recorrerán los procesos hasta el inicio de la cola.\n");
		printf("\t\t\t\t\t      Presione 'enter' para continuar...");
		getchar(); //Para ver la ejecución del programa.
